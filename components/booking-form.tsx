...
export function BookingForm({ 
  ...
  const getAvailabilityDisplay = () => {
    return availability.map(avail => ({
      day: dayNames[avail.day_of_week],
      dayShort: dayNames[avail.day_of_week].slice(0, 3),
    ...
    <div className="space-y-8">
      ...
      <div className="bg-blue-50 rounded-xl p-6 border border-blue-100">
        <div className="flex items-center mb-4">
          ...
          <h3 className="text-lg font-semibold text-blue-900">Available Hours</h3>
          <span className="ml-auto text-sm text-blue-600 bg-blue-100 px-2 py-1 rounded-full">
            {timezone}
          </span>
        </div>
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
          {getAvailabilityDisplay().map((avail) => (
            <div key={avail.dayOfWeek} className="bg-white rounded-lg p-3 border border-blue-200">
              <div className="font-medium text-blue-900 text-sm">{avail.day}</div>
        ...
        {availability.length === 0 && (
          <div className="text-center py-4">
            <p className="text-blue-600">No availability set yet</p>
          </div>
        )}
      </div>
